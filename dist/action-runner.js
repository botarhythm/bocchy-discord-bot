...
const summaryCache = new LRUCache({ max: 256, ttl: 1000 * 60 * 10 }); // 10分
const embeddingCache = new LRUCache({ max: 256, ttl: 1000 * 60 * 10 }); // 10分
...
function buildCharacterPrompt(message, affinity = 0, userProfile = null, globalContext = null) {
    ...
    // 親密度による心理距離
    const relation = affinity > 0.6 ? 'とても親しい' :
...
export async function buildHistoryContext(supabase, userId, channelId, guildId = null, guild = null) {
    ...
    const { data } = await supabase.from('conversation_histories').select('messages').eq('user_id', userId).eq('channel_id', channelId).maybeSingle();
    ...
    if (!sum) {
        const { data: sumData } = await supabase.from('conversation_summaries').select('summary').eq('user_id', userId).eq('channel_id', channelId).order('created_at', { ascending: false }).limit(1).maybeSingle();
        sum = sumData;
        if (sum)
            summaryCache.set(summaryKey, sum);
    }
    // 3) サーバー全体の要約・履歴も取得
    ...
    if (guildId) {
        ...
        if (!gsum) {
            const { data: gsumData } = await supabase.from('conversation_summaries').select('summary').eq('guild_id', guildId).order('created_at', { ascending: false }).limit(1).maybeSingle();
            gsum = gsumData;
            if (gsum)
                summaryCache.set(guildSummaryKey, gsum);
        }
        guildSummary = gsum?.summary;
        const guildRecentKey = `guildRecent:${guildId}`;
        let ghist = summaryCache.get(guildRecentKey);
        if (!ghist) {
            const { data: ghistData } = await supabase.from('conversation_histories').select('messages').eq('guild_id', guildId).order('updated_at', { ascending: false }).limit(10).maybeSingle();
            ghist = ghistData;
            if (ghist)
                summaryCache.set(guildRecentKey, ghist);
        }
    ...
    try {
        ...
        if (!userProfile) {
            const { data: profile } = await supabase.from('user_profiles').select('*').eq('user_id', userId).eq('guild_id', guildId).maybeSingle();
            userProfile = profile;
            if (userProfile)
                summaryCache.set(userProfileKey, userProfile);
        }
    }
    catch (e) {
        userProfile = null;
    ...
    if (guildSummary)
        msgs.push({ role: 'system', content: `【サーバー全体要約】${guildSummary}` });
    if (memberSummary) {
    ...
    try {
        ...
        if (json) {
            ...
            if (obj.topic)
                msgs.unshift({ role: 'system', content: `【会話の流れ要約】${obj.topic}` });
            if (obj.unresolved)
                msgs.unshift({ role: 'system', content: `【未解決の問い】${obj.unresolved}` });
            if (obj.expectation)
                msgs.unshift({ role: 'system', content: `【ユーザーの期待】${obj.expectation}` });
            if (obj.tone)
                msgs.unshift({ role: 'system', content: `【感情トーン】${obj.tone}` });
        }
    }
    catch (e) {
    ...
    try {
        ...
        if (json) {
            const obj = JSON.parse(json);
            if (obj.currentTopic)
                msgs.unshift({ role: 'system', content: `【現在の話題】${obj.currentTopic}` });
            if (obj.currentIssue)
                msgs.unshift({ role: 'system', content: `【直前の課題】${obj.currentIssue}` });
            if (obj.contextMeta)
                msgs.unshift({ role: 'system', content: `【技術的文脈】${obj.contextMeta}` });
        }
    }
    catch (e) {
    ...
    while (totalLength > 5000 && msgs.length > 8) {
        for (let i = 0; i < msgs.length; i++) {
            ...
            if (/【(会話の流れ要約|未解決の問い|ユーザーの期待|感情トーン|長期記憶要約|ユーザーの意図推察|現在の話題|直前の課題|技術的文脈|多角的推論)】/.test(msgs[i].content))
                continue;
...
export async function summarizeWebPage(rawText, userPrompt = '', message = null, charPrompt = null) {
    ...
    const prompt = `以下はWebページの内容です。重要なポイント・要旨・特徴を日本語で分かりやすく要約してください。` +
...
async function enhancedSearch(userPrompt, message, affinity, supabase) {
    ...
    let queries = [];
    for (let i = 0; i < 3; i++) {
        let q = await llmRespond(userPrompt, queryGenSystemPrompt + `\n【バリエーション${i + 1}】できるだけ異なる切り口で。`, message, [], buildCharacterPrompt(message, affinity));
    ...
    let pageContents = await Promise.all(allResults.map(async (r) => {
        try {
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 10000);
            try {
    ...
    const relPrompt = (query, title, snippet) => `ユーザーの質問:「${query}」\n検索結果タイトル:「${title}」\nスニペット:「${snippet}」\nこの検索結果は質問に直接関係していますか？関係が深い場合は「はい」、そうでなければ「いいえ」とだけ返答してください。`;
    const relChecks = await Promise.all(pageContents.map(async (pg) => {
    ...
    let systemPrompt = `あなたはWeb検索アシスタントです。以下の検索結果を比較し、共通点・矛盾点・重要な違いがあれば明示してください。` +
    ...
    if (supabase)
        await saveHistory(supabase, message, `[検索クエリ] ${queries[0]}`, docs, affinity);
    ...
    try {
        ...
        if (referPrevEntityPattern.test(message.content)) {
            ...
            for (let i = history.length - 1; i >= 0; i--) {
                ...
                if (prevPerson && prevEvent && prevSport)
                    break;
            }
            // system promptに明示的に挿入（冒頭に必ず）
            let sysMsg = '';
            if (prevPerson)
                sysMsg += `【直前のperson】${prevPerson}\n`;
            if (prevEvent)
                sysMsg += `【直前のevent】${prevEvent}\n`;
            if (prevSport)
                sysMsg += `【直前のsport】${prevSport}\n`;
            if (sysMsg) {
        ...
        if (supabase)
            await updateAffinity(userId, guildId, message.content);
        if (supabase)
            await saveHistory(supabase, message, message.content, answer, affinity);
    }
    catch (err) {
...
export async function shouldContextuallyIntervene(history, globalContext = null) {
    ...
    let contextStr = '';
    if (globalContext) {
        if (globalContext.topics?.length)
            contextStr += `主な話題: ${globalContext.topics.join('、')}\n`;
        if (globalContext.tone)
            contextStr += `感情トーン: ${globalContext.tone}\n`;
        if (globalContext.summary)
            contextStr += `要約: ${globalContext.summary}\n`;
    }
...
async function getGuildMemberNames(guild, limit) {
